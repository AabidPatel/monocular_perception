...
class VisualOdometry:
    ...
    def get_matches(self, i):
        ...
        frame1 = self.images[i-1]
        frame2 = self.images[i]

        feature_points1 = cv2.goodFeaturesToTrack(frame1, 100, 0.01, 10)
        feature_points2 = cv2.goodFeaturesToTrack(frame2, 100, 0.01, 10)
        ...
        matches = cv2.calcOpticalFlowPyrLK(frame1, frame2, feature_points1, feature_points2, None, None, flags=cv2.OPTFLOW_LK_GET_MIN_EIGENVALS, winSize=(15, 15))

        rotation, translation = cv2.Rodrigues(matches[0][:, :, 2])
...
def main():
    ...
    for i, init_pose in enumerate(cur_poses):
        if i == 0:
            ...
            transf = vo.get_matches(i)
            print("optical flow trans = ", transf)
