...
class KeyboardPlayerPyGame(Player):
    ...
    def act(self):
        global key_events, j, capturing, compute_vlad
        current_key_events = []
    ...
    def see(self, fpv):
        global i, navigating, counter, target_loc, dataset_images, database_vlads, compute_vlad, cluster_centers
        ...
        if(s is not None):
            ...
            if(s[1].value == 1):
                if capturing:
                    ...
                    dataset_images.append(cv2.cvtColor(fpv, cv2.COLOR_BGR2GRAY))
                    i += 1
                elif compute_vlad:
                    print("number of images", len(dataset_images))
                    print("\nComputing Dataset Descriptors")
                    dataset_descriptors = [vlad.extract_descriptors_img(img) for img in dataset_images]
                    print("\n Concatenating Data Discrisptors")
                    all_descriptors = np.concatenate(dataset_descriptors, axis=0)
                    k = 64
                    print("\nComputing kmeans")
                    ...
                    for image in dataset_images:
                        print("z : ", z)
                        database_descriptor = dataset_descriptors[z]
                        database_vlad = vlad.compute_vlad(database_descriptor, cluster_centers)
                ...
                if not navigating:
                    query_descriptors = [vlad.extract_descriptors_img(img) for img in target_images]
                    query_vlads = [vlad.compute_vlad(query_descriptor, cluster_centers) for query_descriptor in query_descriptors]
                    for i, query_vlad in enumerate(query_vlads):
                        similarities = np.dot(database_vlads, query_vlad)
                        target_loc = np.argmax(similarities)
                    # target_loc = self.get_target_location(target_images)
                    print("match =", target_loc)
